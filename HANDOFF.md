...
| آخرین کامیت | `1b2b27c` — «style(inbox): tighter rows, the unread count back, and one palette in the conversation» |
| آخرین بیلد | **سبز** (`run 35586030976`) |
...
0. **ادامه‌ی یکپارچگی بصری (سه دور: `2ba22fd`، `07d0bf1`، `1b2b27c`).**
   انجام‌شده: پالت واحد، چیپ‌های ۴۶dp، BottomNav سفید ۷۲dp با سه آیتم (پیام‌ها /
   مخاطبین / تنظیمات)، ردیف مطابق مرجع: آواتار ۴۸dp سمت **راست** و وسط‌چین، نام،
   زمان در ستون ثابت `48dp`، پیش‌نمایش دو خط `12.5sp`، Badge در خط خودش، ارتفاع
   `84..96dp`، و نقطه‌ی خوانده‌نشده آبی با شمارش برگشت.
   صفحه‌ی گفتگو هم هم‌سان شد: جداکننده‌ی روز به شکل کپسول خاکستری، زمان به‌صورت
   کپشن هم‌تراز با حباب خودش، پالت حباب خاکستری/آبی روشن.
   باگ‌هایی که در عکس گوشی کاربر دیده شد و رفع شد: پیل خالی خاکستری، Badge تبلیغاتی
   بی‌فایده، آواتار مربعی برای فرستندهی بی‌نام، نبود آیکون برند.
   **باقی‌مانده:** صفحه‌های مدیریت/قوانین هنوز با توکن‌های جدید بازبینی نشده‌اند؛
   لوگوی واقعی برندها اضافه نشده (فعلاً آیکون Material + رنگ برند).
   **نکته:** همیشه با اسکرین‌شات کاربر مقایسه کن، نه با حدس.
