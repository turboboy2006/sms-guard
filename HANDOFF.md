...
| آخرین کامیت | `a65b468` — «style(manager,rules): finish the pass on the two screens that were left behind» |
| آخرین بیلد | **سبز** (`run 35588299313`) |
...
0. **یکپارچگی بصری — تمام صفحه‌ها (چهار دور: `2ba22fd`، `07d0bf1`، `1b2b27c`، `a65b468`).**
   همه‌ی صفحه‌ها روی یک توکن نشستند: صندوق پیام، گفتگو، تنظیمات، ظاهر، برندها/مسدودها،
   و قوانین (که قبلاً نوار آبی دومی داشت و حالا سفید است).
   ردیف صندوق مطابق مرجع: آواتار ۴۸dp راست و وسط‌چین، زمان در ستون `48dp`، پیش‌نمایش
   دو خط `12.5sp`، Badge در خط خودش، ارتفاع `84..96dp`، نشانگر خوانده‌نشده آبی با شمارش.
   زبان آواتار یکسان در همه‌جا: کسب‌وکار = رنگ خودش + آیکون سفید، شخص = دایره‌ی پاستلی
   با حرف اول، بی‌نام = دایره‌ی خاکستری با آیکون Person.
   **باقی‌مانده‌ی واقعی:** (الف) تست روی دستگاه — هیچ‌کدام از این‌ها روی گوشی سنجیده نشده؛
   (ب) لوگوی واقعی برندها اضافه نشده (آیکون Material + رنگ برند)؛ (ج) فهرست ویژگی‌های
   هوشی در بخش ۲ زیر.
