...
| آخرین کامیت | `07d0bf1` — «style(inbox): centre the avatar and tighten the row to the reference height» |
| آخرین بیلد | **سبز** (`run 35585352367`) |
...
0. **ادامه‌ی یکپارچگی بصری (بازخورد کاربر — دو دور انجام شد، `2ba22fd` و `07d0bf1`).**
   انجام‌شده: پالت واحد، چیپ‌های ۴۶dp، BottomNav سفید ۷۲dp با سه آیتم (پیام‌ها /
   مخاطبین / تنظیمات)، ردیف مطابق مرجع: آواتار ۵۲dp سمت **راست** و وسط‌چین، نام،
   زمان در ستون ثابت، پیش‌نمایش دو خط، Badge در خط خودش زیر پیش‌نمایش.
   باگ‌هایی که در عکس گوشی کاربر دیده شد و رفع شد: پیل خالی خاکستری (Badge برای دسته‌های
   بی‌برچسب)، Badge تبلیغاتی بی‌فایده، آواتار مربعی برای فرستندهی بی‌نام، نبود آیکون برند.
   **باقی‌مانده‌ی شناخته‌شده:** نقطه‌ی خوانده‌نشده حذف شد (در مرجع هست، زیر زمان) — اگر
   کاربر خواست برگردانده شود؛ ارتفاع ردیف‌ها هنوز ممکن است کمی بیشتر از مرجع باشد.
   **نکته:** با اسکرین‌شات کاربر مقایسه کن، نه با حدس.
